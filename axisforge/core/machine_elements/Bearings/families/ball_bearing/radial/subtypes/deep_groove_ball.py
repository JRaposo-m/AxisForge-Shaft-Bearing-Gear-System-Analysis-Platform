"""
core/machine_elements/Bearings/families/ball/radial/subtypes/deep_groove_ball.py
...
only, not alpha_0_deg -- that's angular_contact.py's idiom). These
constants are declared here, not in ../functions/, because they're this
subtype's own table-row data, not generic math -- angular_contact.py
happens to read similar values right now but declares its own copy,
deliberately, so the two don't depend on each other.

All contact math (parameterised, no baked-in numbers) is in
../functions/contact_stiffness.py.

Manufacturer ri/re override: not supported, matches the actual behaviour
of the pre-rewrite file (its class docstring showed a
setup_internal_geometry(ri=..., re=...) example, but the real method
signature never accepted ri/re -- the docstring was stale, confirmed
against the source).
...
class DeepGrooveBallFamily(BearingFamily):
    ...
    # ri/re don't vary with row count; lambda does -- single row is a
    # separate table row (0.95) from double row (0.90). Subtype input,
    # not generic math -- see module docstring.
    RI_OVER_DW = 0.52
    RE_OVER_DW = 0.52
    ...
    def assemble_geometry(self,
        ...
        i             : number of rows -- 1 (single, default) or 2
                        (double). Selects lambda from
                        REDUCTION_FACTOR_BY_ROWS; ri/re don't depend on it.
        """
    ...
    # Q_ci/Q_ce (eq.19-20) is the same formula for any radial ball
    # bearing -- DGBB and AngularContactFamily both call it identically.
    # dynamic_capacity()/static_capacity() are still NotImplementedError
    # upstream (Formula 15/20/25 text not provided yet).
    # ------------------------------------------------------------------
