...
class ThrustCylindricalRollerFamily(BearingFamily):
    ...
    # ISO 281:2007 Table 2, Table No. 10
    ...
    @property
    def eta_from_alpha_0(alpha_0: float) -> float:
        """eta -- ISO 281:2007 Table 2, Table No. 10, thrust roller bearings,
        alpha_0 != 90deg. alpha_0 in radians."""
        if not (np.pi/4 < alpha_0 < np.pi / 2):
            raise ValueError(f"ThrustCylindricalRollerFamily: alpha_0 must be in (pi/4, pi/2), got {alpha_0}.")
    ...
    @staticmethod
    def dynamic_capacity(bearing, reduction_factor: float | None = None) -> float:
        """
        ...
        reduction_factor : Formula (42) inputs -- ISO 281:2007
        ...
        """
        eta_val = ThrustCylindricalRollerFamily.eta_from_alpha_0(bearing.alpha_0)
        if np.isclose(bearing.alpha_0, np.pi / 2, atol=1e-9):
            ...
            return rcap.BearingCapacity.dynamic_90deg(
                ...
                reduction_factor=reduction_factor if reduction_factor is not None else ThrustCylindricalRollerFamily.LAMBDA_V_THRUST, 
                eta=eta_val,
            )
        return rcap.BearingCapacity.dynamic_nonzero_alpha(
                Z=bearing.Z, Dwe=bearing.Dwe, Lwe=bearing.Lwe, gamma=bearing.gamma,
                reduction_factor=reduction_factor if reduction_factor is not None else ThrustCylindricalRollerFamily.LAMBDA_V_THRUST, 
                eta=eta_val,
            )
