...
class InternalGearMeshing:
    ...
    def forces(self, T_in: float) -> dict[str, float]:
        """
        Compute meshing forces given the input torque T_in, applied on
        whichever gear is set as `self.driver` ("gear1" or "gear2"):

            Ft = T_in / rl_in
            Fr = Ft·tan(αtw)
            Fa = Ft·tan(β_in)      (zero when the driving gear is spur, β=0)
            Fn = Ft/cos(αtw)
            T_out = T_in · u_eff

        where u_eff = u (=z2/z1) if driver is gear1, or 1/u if driver is gear2.

        Parameters
        ----------
        T_in : input (driving) torque [N·m], applied on self.driver
        """
        if self.driver == "gear1":
            rl_in    = self.gear1_w.rl
            beta_in  = self.gear1.beta
            u_eff    = self.u
        else:  # "gear2"
            rl_in    = self.gear2_w.rl
            beta_in  = self.gear2.beta
            u_eff    = 1.0 / self.u
        ...
        return {"Ft": Ft, "Fr": Fr, "Fa": Fa, "Fn": Fn, "T_out": T_out}
