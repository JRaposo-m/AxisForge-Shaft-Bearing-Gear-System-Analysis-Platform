"""
axisforge/mesh/shaft/element_type/euler_bernoulli.py
...
class EulerBernoulliBeam:
    def stiffness_element(self, elem, shear_theory: str | None = None) -> np.ndarray:
        ...
        k[1, 1] = k[4, 4] = 12 * Bending_const
        k[1, 2] = k[1, 5] = k[2, 1] = k[5, 1] = 6 * Bending_const * le
        k[2, 2] = k[5, 5] = 4 * Bending_const * le**2
        k[4, 1] = k[1, 4] = - 12 * Bending_const
        k[4, 2] = k[2, 4] = k[5, 4] = k[4, 5] = - 6 * Bending_const * le
        k[5, 2] = k[2, 5] = 2 * Bending_const * le**2
        return k
    ...
    def shape_functions(self, zeta: float, elem: Elem) -> np.ndarray:
        ...
        N[1] = 1/4 * (2 - 3*zeta + zeta**2)  # transverse v
        N[2] = le/8 * (1 - zeta - zeta**2 + zeta**3)  # rotation θ
        N[3] = 1/2 * (1 + zeta)  # axial     
        N[4] = 1/4 * (2 + 3*zeta - zeta**2)  # transverse v
    ...
    def deformation_matrix(self, zeta: float, elem: Elem) -> np.ndarray:
        le = elem.length
        J = le / 2.0
        B = np.zeros((2, 6))  # linha 0: axial, linha 1: curvatura

        # deformação axial: du/dx
        B[0, 0] = -1.0 / le
        B[0, 3] =  1.0 / le
        ...
        c = 4.0 / le**2
        B[1, 1] = c * (1.5 * zeta)
        B[1, 2] = c * (le/4.0) * (3*zeta - 1)
        B[1, 4] = c * (-1.5 * zeta)
        B[1, 5] = c * (le/4.0) * (1 + 3*zeta)
