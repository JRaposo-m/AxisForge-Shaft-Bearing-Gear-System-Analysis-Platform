...
Due to its physical idealization, this solver is only valid for co-located rows 
(zero axial offset between contact planes). It is NOT valid for a multi-row bearing 
with axial offsets between rows, which means, only for a multi-row thrust ball bearing with co-located rows. 
...
from axisforge.solvers.machine_elements.bearings.load_distribution.ISO_16281.Ball_Bearing.single_row_solver import (
    ISO16281BallSolver,
    REQUIRED_ATTRS,
)
